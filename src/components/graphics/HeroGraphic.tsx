    ...
    >
      <defs>
        <radialGradient id="heroGradient" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
          ...
          <stop offset="60%" style={{ stopColor: 'hsl(var(--primary))', stopOpacity: 0.4 }} />
      ...
      <circle cx="100" cy="100" r="70" fill="url(#heroGradient)" filter="url(#heroGlow)">
        ...
        <animate attributeName="opacity" from="0.6" to="0.9" dur="3s" repeatCount="indefinite" begin="0s" calcMode="spline" keyTimes="0; 0.5; 1" values="0.7;0.9;0.7" keySplines="0.42 0 0.58 1;0.42 0 0.58 1"/>
      </circle>
      ...
      {[...Array(6)].map((_, i) => (
        <circle
          ...
          strokeWidth="1.5"
          opacity_base={0.25 - i * 0.03}
        >
          <animate attributeName="r" dur={`${6 + i}s`} values={`${80 + i * 8}px; ${85 + i * 10}px; ${80 + i * 8}px`} repeatCount="indefinite" />
          <animate attributeName="opacity" dur={`${5 + i}s`} values={`${0.25 - i * 0.03}; ${0.4 - i * 0.03}; ${0.25 - i * 0.03}`} repeatCount="indefinite" />
        </circle>
      ))}

      <path d="M60 100 A40 40 0 0 1 140 100" stroke="url(#lineGrad1)" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.6">
        <animateTransform attributeName="transform" type="rotate" from="0 100 100" to="360 100 100" dur="15s" repeatCount="indefinite" />
      </path>
      <path d="M100 60 A40 40 0 0 1 100 140" stroke="url(#lineGrad2)" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.6">
        <animateTransform attributeName="transform" type="rotate" from="0 100 100" to="-360 100 100" dur="18s" repeatCount="indefinite" />
      </path>
       <circle cx="100" cy="100" r="30" fill="transparent" stroke="hsl(var(--foreground))" strokeWidth="1" strokeDasharray="3 3" opacity="0.15">
      ...
      <text 
        ...
        style={{ textShadow: '0px 0px 3px hsla(var(--primary), 0.7), 0px 0px 6px hsla(var(--accent), 0.5)' }}
      >
